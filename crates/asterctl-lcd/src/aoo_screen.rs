...
pub const FRAME_BYTES: usize = DISPLAY_SIZE.0 as usize * DISPLAY_SIZE.1 as usize * 2;
...
impl AooScreen {
    ...
    pub fn send_image(&mut self, image: impl ToRgb565) -> anyhow::Result<()> {
        ...
        self.send_rgb565_bytes(img_rgb565)
    }

    /// Send one complete display frame that is already encoded as RGB565 little-endian pixels.
    ///
    /// The display accepts exactly 960 x 376 x 2 bytes per frame. Callers that already render
    /// RGB565 data (for example, a browser canvas) can use this method without converting the
    /// frame through an RGB888 image first.
    pub fn send_rgb565_le(&mut self, frame: &[u8]) -> anyhow::Result<()> {
        if frame.len() != FRAME_BYTES {
            return Err(anyhow!(
                "Invalid RGB565 frame size: expected {FRAME_BYTES} bytes, got {}",
                frame.len()
            ));
        }

        self.send_rgb565_bytes(BytesMut::from(frame))
    }

    fn send_rgb565_bytes(&mut self, img_rgb565: BytesMut) -> anyhow::Result<()> {
        debug!(
...
mod tests {
    ...
    #[test]
    fn raw_rgb565_frame_requires_exact_display_size() {
        let mut screen = AooScreenBuilder::new().simulate().unwrap();
        let error = screen
            .send_rgb565_le(&vec![0_u8; FRAME_BYTES - 1])
            .unwrap_err();
        assert!(error.to_string().contains(&FRAME_BYTES.to_string()));
    }
}
